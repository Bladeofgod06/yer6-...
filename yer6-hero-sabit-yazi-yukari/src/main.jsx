...
async function addDbLog(action, detail, actor='SYSTEM'){
  try{
    ...
    if(error){
      console.error('LOG HATASI:', error.message, payload);
      return false;
    }
...
function canViewBanAppeals(admin) {
  return Number(admin?.level || 0) >= 9;
}
...
function punishmentStartCommands(p) {
  return `${wlGiveCommand(p)}\n${wlBanCommand(p)}`;
}
function punishmentFinishCommands(p) {
  return `${wlFinishRemoveBanCommand(p)}\n${wlFinishGiveWlCommand(p)}`;
}
...
function PlayerPanel({player,setPlayer,setPage,players,setPlayers,tickets,setTickets,apps,setApps,punishments,setPunishments,setLogs,announcements,banAppeals,setBanAppeals,wheelRewards,promoCodes,pendingRewards,setPendingRewards,referrals,setReferrals,events,eventRules,eventParticipants,setEventParticipants}) {
 ...
 async function addTicket() {
  ...
  setTicket({type:'AC (OCEAN) Destek',title:'',description:'',proof:''});
 ...
 async function sendBanAppeal(){
  if(!banAppeal.characterName||!banAppeal.discordId||!banAppeal.eventDate||!banAppeal.reason||!banAppeal.record) return alert('Ban itirazı için tüm alanları doldurmalısın.');
  const payload={username:player.username,discord_id:banAppeal.discordId,character_name:banAppeal.characterName,event_date:banAppeal.eventDate,reason:banAppeal.reason,record:banAppeal.record,status:'Bekliyor'};
  const { data, error } = await supabase.from('ban_appeals').insert(payload).select().single();
  const item=data?dbBanAppealToApp(data):{id:String(Date.now()),username:player.username,discordId:banAppeal.discordId,characterName:banAppeal.characterName,eventDate:banAppeal.eventDate,reason:banAppeal.reason,record:banAppeal.record,status:'Bekliyor',createdAt:now()};
  if(error) console.log('Ban itirazı Supabase kayıt hatası:', error.message);
  setBanAppeals(p=>[item,...(p||[])]);
  setLogs(p=>[now()+' - ban itiraz başvurusu: '+player.username,...p]);
  await addDbLog('BAN_APPEAL_CREATE', `${player.username} ban itiraz başvurusu gönderdi.`, player.username);
  alert('Ban itiraz başvurun gönderildi.');
 ...
 return <div className="adminLayout playerLayout cleanPlayerPanel">
  <aside>
   ...
   <Button variant="ghost" onClick={async()=>{await addDbLog('PLAYER_LOGOUT', `${player.username} oyuncu panelinden çıkış yaptı.`, player.username);localStorage.removeItem('yer6_player_session');setPlayer(null);setPage('home')}}>Çıkış</Button>
  </aside>
  <main>
   ...
   {active==='Kapanan Destekler'&&<Card className="panel"><h2>Kapanan Destekler</h2>{myClosedTickets.length===0&&<p>Kapanan ticket yok.</p>}{myClosedTickets.map(t=><div className="ticketThread compactTicket closedTicket" key={t.id}><div className="row"><div><b>{t.id} • {t.title}</b><p>{t.type} • Kapalı • Yetkili: {t.assigned}</p><small>{t.createdAt}</small></div><Badge tone="bad">Kapalı</Badge></div></div>)}</Card>}

   {active==='Ban İtiraz Başvurusu'&&<Card className="panel"><h2>Ban İtiraz Başvurusu</h2><p>Ban itirazı destek/ticket olarak değil, ayrı başvuru formu olarak gönderilir.</p><Field value={banAppeal.characterName} onChange={v=>setBanAppeal({...banAppeal,characterName:v})} placeholder="[IC] Karakterinizin Adı Soyadı"/><Field value={banAppeal.discordId} onChange={v=>setBanAppeal({...banAppeal,discordId:v})} placeholder="Discord ID"/><Field type="date" value={banAppeal.eventDate} onChange={v=>setBanAppeal({...banAppeal,eventDate:v})} placeholder="Olayın Yaşandığı Tarih"/><TextArea value={banAppeal.reason} onChange={v=>setBanAppeal({...banAppeal,reason:v})} placeholder="Neden Ban Yediniz? (Olayı kısaca özetleyiniz)"/><TextArea value={banAppeal.record} onChange={v=>setBanAppeal({...banAppeal,record:v})} placeholder="Olayla ilgili kaydınız var mı? Varsa link bırakınız."/><Button onClick={sendBanAppeal}><Send size={16}/> Başvuru Gönder</Button></Card>}
...
function AdminPanel({admin,setAdmin,setPage,admins,setAdmins,players,setPlayers,donate,setDonate,staffRanks,setStaffRanks,staffMembers,setStaffMembers,tickets,setTickets,apps,setApps,banAppeals,setBanAppeals,punishments,setPunishments,logs,setLogs,announcements,setAnnouncements,wheelRewards,setWheelRewards,promoCodes,setPromoCodes,pendingRewards,setPendingRewards,referrals,setReferrals,events,setEvents,eventRules,setEventRules,eventParticipants,setEventParticipants}) {
 ...
 return <div className="adminLayout"><aside><Logo/><p>{admin.username} • LVL {admin.level} • {admin.role}</p>{menu.map(m=><button key={m} className={active===m?'active':''} onClick={()=>setActive(m)}>{m}</button>)}<Button variant="ghost" onClick={async()=>{await addDbLog('ADMIN_LOGOUT', `${admin.username} admin panelinden çıkış yaptı.`, admin.username);localStorage.removeItem('yer6_admin_session');setAdmin(null);setPage('home')}}>Çıkış</Button></aside><main><div className="adminTop"><div><h1>{active}</h1><p>Full yönetim paneli</p></div><Button onClick={()=>setLogs(p=>[now()+' - Bildirim kontrol edildi',...p])}><Bell size={16}/> Bildirim</Button></div>
  ...
  {active==='Ban İtirazları'&&canViewBanAppeals(admin)&&<Card className="panel"><h2>Ban İtiraz Başvuruları</h2><p>Bu bölümü Moderatör ve üstü yetkililer görüntüleyebilir.</p>{(!banAppeals||banAppeals.length===0)&&<p>Ban itiraz başvurusu yok.</p>}{(banAppeals||[]).map(a=><div className="row" key={a.id}><div><b>{a.characterName} • {a.username}</b><p>{a.discordId} • {a.status} • Olay Tarihi: {a.eventDate||'Belirtilmedi'}</p><small><b>Özet:</b> {a.reason}</small><br/><small><b>Kayıt:</b> {a.record}</small></div><div className="actions"><Button onClick={()=>banAppealResult(a.id,'Kabul Edildi')}>Kabul</Button><Button variant="ghost" onClick={()=>banAppealResult(a.id,'Reddedildi')}>Reddet</Button></div></div>)}</Card>}
...
<div className="commandPreview commandPreviewFour">
  <b>Discord Rol Komutları</b>

  <div className="commandCards">
    <div className="commandCard">
      <small>Ceza Başlat 1</small>
      <code>{punish.targetId ? wlGiveCommand(punish) : 'Discord ID girince komut oluşur.'}</code>
      <Button variant="ghost" onClick={()=>copyText(wlGiveCommand(punish))}>1. Komutu Kopyala</Button>
    </div>

    <div className="commandCard">
      <small>Ceza Başlat 2</small>
      <code>{punish.targetId ? wlBanCommand(punish) : 'Discord ID girince komut oluşur.'}</code>
      <Button variant="ghost" onClick={()=>copyText(wlBanCommand(punish))}>2. Komutu Kopyala</Button>
    </div>

    <div className="commandCard">
      <small>Ceza Bitir 1</small>
      <code>{punish.targetId ? wlFinishRemoveBanCommand(punish) : 'Discord ID girince komut oluşur.'}</code>
      <Button variant="ghost" onClick={()=>copyText(wlFinishRemoveBanCommand(punish))}>3. Komutu Kopyala</Button>
    </div>

    <div className="commandCard">
      <small>Ceza Bitir 2</small>
      <code>{punish.targetId ? wlFinishGiveWlCommand(punish) : 'Discord ID girince komut oluşur.'}</code>
      <Button variant="ghost" onClick={()=>copyText(wlFinishGiveWlCommand(punish))}>4. Komutu Kopyala</Button>
    </div>
  </div>
</div>

<div className="actions">
  <Button onClick={addPunishment}>Ceza Kaydet</Button>
</div></Card>}
  ...
  <Button variant="ghost" onClick={()=>copyText(wlGiveCommand(p))}>1. Başlat / Whitelisted Al</Button>
  <Button variant="ghost" onClick={()=>copyText(wlBanCommand(p))}>2. Başlat / Uzaklaştırma Ver</Button>
  <Button variant="ghost" onClick={()=>copyText(wlFinishRemoveBanCommand(p))}>3. Bitir / Uzaklaştırma Al</Button>
  <Button variant="ghost" onClick={()=>copyText(wlFinishGiveWlCommand(p))}>4. Bitir / Whitelisted Ver</Button>
  <Button onClick={()=>finishPunishment(p.id)}><CheckCircle size={16}/> WL Geri Ver / Bitir</Button>
</div></div>)}</Card>}
  {active==='Ceza Kayıtları'&&<Card className="panel"><h2>Ceza Kayıtları</h2>{punishments.length===0&&<p>Ceza kaydı yok.</p>}{punishments.map(p=><div className="row" key={p.id}><div><b>{p.id} • {p.targetType} • {p.targetId}</b><p>{p.rule} • {p.penalty} • {p.status}</p><small>Yetkili: {p.by} • {p.createdAt}</small><div className="miniCommand">{wlGiveCommand(p)}</div></div><div className="actions">
  <Button variant="ghost" onClick={()=>copyText(wlGiveCommand(p))}>1. Başlat</Button>
  <Button variant="ghost" onClick={()=>copyText(wlBanCommand(p))}>2. Başlat</Button>
  <Button variant="ghost" onClick={()=>copyText(wlFinishRemoveBanCommand(p))}>3. Bitir</Button>
  <Button variant="ghost" onClick={()=>copyText(wlFinishGiveWlCommand(p))}>4. Bitir</Button>
</div></div>)}</Card>}
...
function App(){
 ...
 useEffect(()=>{
  if(admin?.username && !sessionStorage.getItem('yer6_admin_session_logged')){
   sessionStorage.setItem('yer6_admin_session_logged','1');
   addDbLog('ADMIN_SESSION_ACTIVE', `${admin.username} kayıtlı oturumla admin paneline girdi.`, admin.username);
  }
 },[admin]);

 useEffect(()=>{
  if(player?.username && !sessionStorage.getItem('yer6_player_session_logged')){
   sessionStorage.setItem('yer6_player_session_logged','1');
   addDbLog('PLAYER_SESSION_ACTIVE', `${player.username} kayıtlı oturumla oyuncu paneline girdi.`, player.username);
  }
 },[player]);
 ...
 async function loginAdmin(){
  ...
  try{
   ...
   await addDbLog('ADMIN_LOGIN', `${adminData.username} admin paneline giriş yaptı.`, adminData.username);
   setAdmin(adminData);
 ...
 async function loginPlayer(){
  ...
  try{
   ...
   await addDbLog('PLAYER_LOGIN', `${playerData.username} oyuncu paneline giriş yaptı.`, playerData.username);
 ...
 async function registerPlayer(){
  ...
  try{
   ...
   const playerData = dbPlayerToApp(data);
   await addDbLog('PLAYER_REGISTER', `${playerData.username} kayıt oldu ve oyuncu paneline giriş yaptı.`, playerData.username);
