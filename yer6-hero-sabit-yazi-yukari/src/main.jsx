...
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL || 'https://ptzvjftojukhyeocyhdi.supabase.co';
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY || 'sb_publishable_0I8eHo0eaikKWxwyD6kfOg_Qm7Z_SqG';
...
function normalizeDonateCategory(d){
 ...
 return {
  ...
  cover:d.cover||oldImages.find(Boolean)||products.find(p=>p.image)?.image||'',
  items:products.map(p=>p.name),
  images:products.slice(0,4).map(p=>p.image||''),
  products
 };
}
...
function MarketPage({setPage,openLogin,donate}) {
 ...
 return <div className="inner donateLuxuryPage"><Header setPage={setPage} openLogin={openLogin}/><main>
  <section className="donateLuxuryHero">
   <div><span>YER6 DONATE MARKET</span><h1>Lüks Donate Koleksiyonu</h1><p>Kategorilere tıkla, araçları ve özel paketleri panel olarak görüntüle. Fiyat yazmaz; satın alma Discord üzerinden yapılır.</p><div className="heroButtons"><Button onClick={()=>window.open(discordUrl,'_blank')}>Discord'dan Satın Al</Button><Button variant="ghost" onClick={()=>setPage('home')}>Ana Sayfa</Button></div></div>
   <Card className="donateLuxuryInfo"><Crown size={40}/><h2>Özel Satış</h2><p>Stok, fiyat ve teslimat bilgisi Discord üzerinden yetkililer tarafından verilir.</p></Card>
  </section>
  <section className="donateLuxuryGrid">{market.map((d,i)=>{const cover=d.cover||(d.products||[]).find(p=>p.image)?.image;return <Card className="donateLuxuryCard" key={d.type+i}><div className="donateLuxuryImg">{cover?<img src={cover} alt={d.type}/>:<ShoppingCart size={38}/>}</div><div className="donateLuxuryBody"><small>DONATE KATEGORİ</small><h2>{d.type}</h2><p>{d.desc}</p><Button onClick={()=>setSelected(d)}>Ürünleri Görüntüle</Button></div></Card>})}</section>
  {selected&&<div className="donateProductOverlay"><Card className="donateProductModal"><div className="donateProductHead"><div><span>DONATE ÜRÜNLERİ</span><h2>{selected.type}</h2><p>{selected.desc}</p></div><button className="donateClose" onClick={()=>setSelected(null)}>×</button></div><div className="donateProductGrid">{(selected.products||[]).map((p,i)=><Card className="donateProductCard" key={(p.name||'urun')+i}><div className="donateProductPhoto">{p.image?<img src={p.image} alt={p.name}/>:<Star size={32}/>}</div><div className="donateProductText"><small>{selected.type}</small><h3>{p.name}</h3><p>{p.desc||'Detay ve satın alma için Discord üzerinden yetkiliyle görüş.'}</p><Button onClick={()=>window.open(discordUrl,'_blank')}>Discord'dan Satın Al</Button></div></Card>)}{(!selected.products||selected.products.length===0)&&<Card className="donateProductCard empty"><div className="donateProductText"><h3>Bu kategoriye ürün eklenmedi</h3><p>Admin panelinden kart ekleyebilirsin.</p></div></Card>}</div></Card></div>}
 </main></div>
}
...
function App(){
 ...
 async function loginAdmin(){
  ...
  try{
   ...
   if(!error && data){const a=dbAdminToApp(data);setAdmin(a);localStorage.setItem('yer6_session',JSON.stringify({kind:'admin',user:a}));setPage('admin');await addDbLog('ADMIN_LOGIN',`${a.username} giriş yaptı.`,a.username);return;}
  ...
  if(!a)return alert('Admin bilgileri yanlış.');
  setAdmin(a);localStorage.setItem('yer6_session',JSON.stringify({kind:'admin',user:a}));setPage('admin');
 }
 async function loginPlayer(){
  if(!auth.discordId || !auth.password) return alert('Discord ID ve şifre gerekli.');
  try{
   const { data, error } = await supabase.from('players').select('*').eq('discord_id',auth.discordId).eq('password',auth.password).single();
   if(!error && data){const p=dbPlayerToApp(data);setPlayer(p);localStorage.setItem('yer6_session',JSON.stringify({kind:'player',user:p}));setPage('player');return;}
  }catch(e){console.log(e)}
  ...
  if(!p)return alert('Oyuncu bilgileri yanlış.');
  setPlayer(p);localStorage.setItem('yer6_session',JSON.stringify({kind:'player',user:p}));setPage('player');
 }
 async function registerPlayer(){
  if(!auth.username||!auth.discordId||!auth.password)return alert('Kullanıcı adı, Discord ID ve şifre gerekli.');
  try{
   const { data, error } = await supabase.from('players').insert({username:auth.username,discord_id:auth.discordId,password:auth.password,steam:auth.steam||'',wl_status:'Aktif'}).select().single();
   if(!error && data){const p=dbPlayerToApp(data);setPlayers(prev=>[p,...prev.filter(x=>String(x.discordId)!==String(p.discordId))]);setPlayer(p);localStorage.setItem('yer6_session',JSON.stringify({kind:'player',user:p}));setPage('player');await addDbLog('PLAYER_REGISTER',`${p.username} kayıt oldu.`,p.username);return;}
   if(error && !String(error.message||'').toLowerCase().includes('duplicate')) alert('Supabase kayıt uyarısı: '+error.message+' - local kayıt açılıyor.');
  }catch(e){console.log(e)}
  const exists=players.find(x=>String(x.discordId)===String(auth.discordId));
  if(exists)return alert('Bu Discord ID ile kayıt var. Oyuncu girişi yap.');
  const p={username:auth.username,discordId:auth.discordId,password:auth.password,steam:auth.steam||'',wlStatus:'Aktif',wlEndDate:'',banReason:''};
  setPlayers(prev=>[p,...prev]);setPlayer(p);localStorage.setItem('yer6_session',JSON.stringify({kind:'player',user:p}));setPage('player');
 }
